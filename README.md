...
1. Copy this whole repo into your HA config's `addons/` folder (or add it as
   a repository under **Settings -> Add-ons -> Add-on Store -> ⋮ ->
   Repositories**).
2. Install "WhatsApp Reminder Platform" from the Local Add-ons list.
...
5. Wire up your dashboard using `ha-addon/dashboard-example.yaml`, which
   adapts your existing mushroom-card dashboard to call the new API instead
   of the old Apps Script web app - same look, same helpers, new backend.
